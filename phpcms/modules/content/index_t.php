...
class index_t {
	...
	public function init() {
		...
		include template('content','index_t',$default_style);
	}
...
//add by michaellee date:2013-7-11 start
	...
	private function getRss(){
		...
		$ch = curl_init();
		curl_setopt ($ch, CURLOPT_URL, 'http://www.zmodo.com.cn/blog/rss');
	...
	public function getNewNews(){
		...
		while( $row = mysql_fetch_array($result)){
			...
			foreach($tmpData['rss']['channel']['item']	as	$k=>$v){
				...
				$title=str_replace(']]>','',$title);
				$datab[$k]['title']=$title;
				$datab[$k]['url']=APP_PATH.'index.php?m=content&c=index&a=zmodocn&id='.$k;
				$datab[$k]['udate']=strtotime($v['pubDate']);
				$datas=str_replace('<![CDATA[','',$v['description']);
				$datas=str_replace(']]>','',$datas);
				$datab[$k]['content']=$datas;
				unset($tmpData[$k]);
				if($k==9) break;
			}
		$data=array_merge($data1,$datab);
		foreach($data 	as	$k=>$v){
